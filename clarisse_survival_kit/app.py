        ...
        if asset_type == '3d':
            ...
            if files:
                ...
                if textures.get('opacity'):
                    ix.cmds.SetShadingLayerRulesProperty(str(shading_layer), [0], "clip_map", [str(surface.get('opacity'))])
...
def mix_surfaces(ctx1, ctx2, mix_surface_name="mix" + MATERIAL_SUFFIX,
    ...
    if has_displacement:
        ...
        displacement1_blend_offset_tx = ix.cmds.CreateObject(surface1_name + DISPLACEMENT_BLEND_OFFSET_SUFFIX, "TextureAdd",
                                                             "Global",
                                                             str(ctx))
        ix.cmds.SetTexture([str(displacement1_blend_offset_tx) + ".input1"], str(displacement1_height_scale_tx))
        displacement1_offset_tx = ix.cmds.CreateObject(surface1_name + DISPLACEMENT_OFFSET_SUFFIX, "TextureAdd", "Global",
                                                       str(ctx))
        ...
        displacement2_blend_offset_tx = ix.cmds.CreateObject(surface2_name + DISPLACEMENT_BLEND_OFFSET_SUFFIX, "TextureAdd",
                                                             "Global",
                                                             str(ctx))
        ix.cmds.SetTexture([str(displacement2_blend_offset_tx) + ".input1"], str(displacement2_height_scale_tx))
        displacement2_offset_tx = ix.cmds.CreateObject(surface2_name + DISPLACEMENT_OFFSET_SUFFIX, "TextureAdd", "Global",
                                                       str(ctx))
...
def import_ms_library(library_dir, target_ctx=None, custom_assets=True, skip_categories=(), **kwargs):
    ...
    Custom textures are ignored.
    If you want to import those set the path to the My Assets folder.
    TODO: make ui and folder structure
    """
    ix = get_ix(kwargs.get("ix"))
    if not target_ctx:
        target_ctx = ix.application.get_working_context()
